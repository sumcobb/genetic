...
void Robby::initializeField(int matrix[10][10])
{
    ...
    for ( int i = 0; i < 10; i++ )
    {
        ...
        {
            field [ j ] [ i ] = 1;
            if ((rand() % 2 == 0) && (field[j][i] == 1) && (cans < 50))
            { // ca 50 av rutorna blir burkar
                field [ j ] [ i ] = 2;
                cans++;
...
void Robby::makeMove(int action, bool showMoves = false)
{
    int c, n, e, s, w;
    ...
    if (showMoves)
    {
        cout << endl << "ROBBY: current position:";
        printPos();
    ...
    switch (action)
    {
        ...
        case 6: // random move
            if ((x_pos + x_increment < 0) || (x_pos + x_increment > 9) ||
                (y_pos + y_increment < 0) || (y_pos + y_increment > 9))
            {
                // wall would be hit
                penalties++;
                points -= 5;
